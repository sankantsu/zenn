---
title: "Zenn の記事選択機能を題材にした telescope.nvim 拡張機能作成ハンズオン"
emoji: "🛠️"
type: "tech"
...
- `telescope.nvim` でどんなことができるのか知りたい
- `telescope.nvim` の拡張を書いてみたい

という方は是非読んでみてください。

作成した拡張機能は以下に置いてあります。ご興味があればこちらも使ってみていただければ嬉しいです。
...
これだけたと抽象的に見えますが、豊富な検索対象 + それに対する操作が builtin ライブラリとして提供されており、たとえば
...
記事の slug, タイトル等を入力データとして扱うためにはこれらのデータをなにがしかの方法でとってきてやる必要があります。これは基本的には `articles/` 以下のディレクトリにある markdown ファイル全部を集めてきて、front matter をパースした結果を出力するという機構が必要になります。Front matter は基本的に `---` の区切り行で挟まれた yaml なので、この部分を切り出して yaml パーサにかければ良いです。
